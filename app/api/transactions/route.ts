...
export async function GET(req: Request) {
  try {
    ...
    if (search) {
      conditions.push(`(account ILIKE $${params.length + 1} OR details ILIKE $${params.length + 1} OR amount ILIKE $${params.length + 1})`);
      params.push(`%${search}%`);
    }
    ...
    if (sort) {
      if (sort === "momo") {
        query += " WHERE tr_type = 'momo'";
      } else if (sort === "bank") {
        query += " WHERE tr_type = 'bank'";
      }else if (sort === "cash") {
        query += " WHERE tr_type = 'cash'";
      } else if (sort === "card") {
        query += " WHERE tr_type = 'card'";
      } else if (sort === "category") {
        query += " ORDER BY tr_type ASC";
      }
    } else {
